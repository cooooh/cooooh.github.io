...
### 网页写文章（入口：分类页「写文章」按钮，或直接访问 /write/）
...
**添加资源（网页操作）**：打开分区页 → 点右上角「＋ 添加资源」→ 填写表单保存即可
（原理与「写文章」相同：通过 GitHub API 写入数据文件 `source/js/resource-data/`，1~2 分钟自动部署上线；发布密钥与写文章页共用）。

**手动编辑数据文件**也可以：`source/js/resource-data/anime.js`（番剧）或 `novel.js`（小说），
按文件顶部注释的格式复制一条 `{ ... }` 块填写即可：

- `name` 名称（必填）· `cover` 封面（网络链接或 `/img/xxx.jpg`，留空显示占位封面）· `descr` 一句话简介 · `tags` 标签数组 · `links` 链接按钮数组
- 页面结构：`source/anime/index.md`、`source/novel/index.md`；渲染脚本 `source/js/resource-page.js`；添加资源页 `source/resource-add/`（脚本 `resource-add.js`）；样式 `source/css/resource.css`
...
保存过发布密钥的浏览器（也就是你自己）会多出删除能力，普通访客完全看不到：

- 页面左下角出现「**管理**」按钮，点击进入管理模式
- **文章**：首页/归档/分类的文章卡片右上角出现红色删除角标；文章详情页左下角出现「删除本文」按钮
- **资源**：番剧/小说页的卡片右上角出现删除角标（管理模式开启后显示）
- 删除操作会通过 GitHub API 移除仓库里对应文件，1~2 分钟后全站生效（删除文章后自动跳回首页）

相关代码：删除逻辑 `source/js/delete-manager.js`（文章）与 `source/js/resource-page.js`（资源），
文章源文件路径注入由 `scripts/post-source.js` 完成，样式 `source/css/delete-manager.css`。
