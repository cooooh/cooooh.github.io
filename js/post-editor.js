 ...
 *
 * 令牌（fine-grained PAT，仅本仓库 Contents 读写权限）保存在浏览器 localStorage。
 */
;(() => {
  'use strict'
  ...
  const refreshTokenUI = () => {
    ...
    const summary = tokenBox ? tokenBox.querySelector('summary') : null
    if (summary) {
      summary.innerHTML = token
        ? '🔑 GitHub 令牌：<span class="pe-token-ok">已保存 ✓</span>（点击可更换）'
        : '🔑 GitHub 令牌设置（首次使用必读）'
    }
    if (tokenRemoveBtn) tokenRemoveBtn.style.display = token ? '' : 'none'
    if (tokenInput) tokenInput.value = ''
  }

  if (tokenBox) tokenBox.addEventListener('toggle', () => { if (tokenBox.open && tokenInput) tokenInput.focus() })
  ...
  if (tokenSaveBtn) {
    tokenSaveBtn.addEventListener('click', () => {
      ...
      tokenInput.value = ''
      setStatus($('pe-token-status'), '令牌已保存 ✓', true)
      refreshTokenUI()
    })
  }
  ...
  if (tokenRemoveBtn) {
    tokenRemoveBtn.addEventListener('click', () => {
      ...
      setStatus($('pe-token-status'), '令牌已移除', true)
      refreshTokenUI()
    })
  }
  ...
  if (publishBtn) {
    publishBtn.addEventListener('click', async () => {
      const token = getToken()
      if (!token) {
        if (tokenBox) tokenBox.open = true
        setStatus(statusEl, '请先在上方保存 GitHub 令牌', false)
        return
      }
